    ...
    if ( 'book_creator' === $post_type ) {
        return array(
            ...
            'bc_subtitle'     => array(
                ...
                'type'              => 'text',
                'source'            => 'meta',
                'sanitize_callback' => 'bookcreator_passthrough_sanitize',
                'format'            => 'text',
            ),
            'bc_author'       => array(
...
function bookcreator_passthrough_sanitize( $value ) {
...
function bookcreator_get_book_content_meta_schema() {
    return array(
        'bc_subtitle'        => 'bookcreator_passthrough_sanitize',
...
function bookcreator_get_epub_designer_field_definitions() {
    return array(
        ...
        'post_title'        => array(
            'label' => __( 'Titolo Libro', 'bookcreator' ),
        ),
        'bc_subtitle'       => array(
            'label' => __( 'Sottotitolo', 'bookcreator' ),
        ),
        'publisher_image'   => array(
        ...
        'chapter_title'     => array(
            'label' => __( 'Titolo Capitolo', 'bookcreator' ),
        ),
        'chapter_content'   => array(
            'label' => __( 'Contenuto Capitolo', 'bookcreator' ),
        ),
        'paragraph_title'   => array(
            'label' => __( 'Titolo Paragrafo', 'bookcreator' ),
        ),
        'paragraph_content' => array(
            'label' => __( 'Contenuto Paragrafo', 'bookcreator' ),
        ),
        'bc_footnotes'      => array(
...
function bookcreator_get_epub_designer_selector_map() {
    return array(
        'bc_author'         => array( '.bookcreator-book-header__author' ),
        'bc_coauthors'      => array( '.bookcreator-book-header__coauthors' ),
        'post_title'        => array( '.bookcreator-book-header__title', '.bookcreator-book-title' ),
        'bc_subtitle'       => array( '.bookcreator-book-header__subtitle' ),
        'publisher_image'   => array( '.bookcreator-book-header__publisher-logo-image', '.bookcreator-book-header__publisher-logo img' ),
        ...
        'paragraph_title'   => array( '.bookcreator-paragraph__title' ),
...
function bookcreator_get_epub_designer_template_mapping() {
    return array(
        'bc_author'         => 'book_author',
        'bc_coauthors'      => 'book_coauthors',
...
function bookcreator_get_epub_designer_default_order() {
...
function bookcreator_sanitize_css_value( $value ) {
    ...
    if ( preg_match( '/^[0-9.,\-\s%]+(px|rem|em|mm|pt|vw|vh|%)?$/i', $value ) ) {
        return $value;
    }
...
function bookcreator_normalize_epub_designer_field_styles( $styles ) {
    ...
    foreach ( $styles as $property => $value ) {
        $property = strtolower( preg_replace( '/[^a-z0-9\-]/', '', str_replace( '_', '-', $property ) ) );
        if ( ! in_array( $property, $allowed_properties, true ) ) {
            continue;
        }

        $normalized[ $property ] = bookcreator_sanitize_css_value( $value );
    }

    return $normalized;
}

function bookcreator_normalize_epub_designer_settings( $raw_settings ) {
    $fields_config = bookcreator_get_epub_designer_field_definitions();
    $defaults      = bookcreator_get_epub_designer_default_order();
    ...
    if ( isset( $raw_settings['fields'] ) && is_array( $raw_settings['fields'] ) ) {
        foreach ( $raw_settings['fields'] as $field ) {
            ...
            $fields[ $field_id ] = array(
                ...
                'label'   => isset( $field['label'] ) ? sanitize_text_field( $field['label'] ) : $fields_config[ $field_id ]['label'],
                'visible' => isset( $field['visible'] ) ? (bool) $field['visible'] : true,
                'styles'  => bookcreator_normalize_epub_designer_field_styles( isset( $field['styles'] ) ? $field['styles'] : array() ),
            );
        }
    }
...
function bookcreator_template_has_designer_settings( $template ) {
    return isset( $template['settings']['designer'] ) && is_array( $template['settings']['designer'] );
}

function bookcreator_get_template_designer_settings( $template ) {
    if ( ! bookcreator_template_has_designer_settings( $template ) ) {
        return null;
    }
...
function bookcreator_epub_designer_field_visible( $designer_settings, $field_id, $default = true ) {
    if ( ! $designer_settings || ! isset( $designer_settings['fields'] ) ) {
        return $default;
    }

    $field_id = sanitize_key( $field_id );
    if ( isset( $designer_settings['fields'][ $field_id ] ) ) {
        return ! empty( $designer_settings['fields'][ $field_id ]['visible'] );
    }
...
function bookcreator_generate_epub_designer_css_rules( $designer_settings ) {
    ...
    $selectors_map = bookcreator_get_epub_designer_selector_map();
    $css_rules     = array();
...
function bookcreator_get_pdf_font_family_options() {
        ...
        try {
            ...
            if ( isset( $defaults['fontdata'] ) && is_array( $defaults['fontdata'] ) ) {
                foreach ( $defaults['fontdata'] as $font_key => $font_config ) {
                    ...
                    $variant_files = array();
                    foreach ( array( 'R', 'B', 'I', 'BI' ) as $variant_key ) {
                        if ( isset( $font_config[ $variant_key ] ) && is_string( $font_config[ $variant_key ] ) ) {
                            $file_name = trim( $font_config[ $variant_key ] );
                            if ( '' !== $file_name ) {
                                $variant_files[ $variant_key ] = $file_name;
                            }
                        }
                    }
    ...
    foreach ( $fonts as $font_key => $font_data ) {
        ...
        if ( ! isset( $fonts[ $font_key ]['files'] ) || ! is_array( $fonts[ $font_key ]['files'] ) ) {
            $fonts[ $font_key ]['files'] = array();
        }
    }
...
function bookcreator_generate_pdf_font_face_css() {
    static $css = null;
    ...
    $fonts = bookcreator_get_pdf_font_family_options();
    if ( empty( $fonts ) ) {
        return $css;
    }

    $font_dir      = trailingslashit( plugin_dir_path( __FILE__ ) ) . 'mpdf-8-1-0/ttfonts/';
    $font_url_base = trailingslashit( plugin_dir_url( __FILE__ ) ) . 'mpdf-8-1-0/ttfonts/';
    ...
    foreach ( $fonts as $font_key => $font_data ) {
        if ( empty( $font_data['files'] ) || ! is_array( $font_data['files'] ) ) {
            continue;
        }

        $family_names = array();

        $font_key_name = isset( $font_data['mpdf_key'] ) ? (string) $font_data['mpdf_key'] : (string) $font_key;
        if ( '' !== $font_key_name ) {
            $family_names[] = $font_key_name;
        }

        $label = isset( $font_data['label'] ) ? trim( (string) $font_data['label'] ) : '';
        if ( '' !== $label ) {
            $family_names[] = $label;
        }
        ...
        foreach ( $font_data['files'] as $variant => $file_name ) {
            ...
            if ( ! in_array( $extension, array( 'ttf', 'otf', 'ttc' ), true ) ) {
                continue;
            }

            $style  = isset( $variant_map[ $variant ] ) ? $variant_map[ $variant ]['style'] : 'normal';
            $weight = isset( $variant_map[ $variant ] ) ? $variant_map[ $variant ]['weight'] : '400';

            $font_url = $font_url_base . str_replace( '%2F', '/', rawurlencode( $file_name ) );
            $format   = ( 'otf' === $extension ) ? 'opentype' : 'truetype';

            foreach ( $family_names as $family_name ) {
                $family_key = strtolower( $family_name );
                if ( ! isset( $registered[ $family_key ] ) ) {
                    $registered[ $family_key ] = array();
                }

                $style_key = $style . '|' . $weight;
                if ( isset( $registered[ $family_key ][ $style_key ] ) ) {
                    continue;
                }

                $registered[ $family_key ][ $style_key ] = true;

                $escaped_family = str_replace(
                    array( '\\', "'" ),
                    array( '\\\\', "\\'" ),
                    $family_name
                );

                $rules[] = "@font-face {\n  font-family: '" . $escaped_family . "';\n  font-style: $style;\n  font-weight: $weight;\n  src: url('" . esc_url_raw( $font_url ) . "') format('$format');\n  font-display: swap;\n}";
            }
        }
    }
...
function bookcreator_get_epub_style_fields() {
    return array(
        'book_title' => array(
            ...
            'selectors' => array( '.bookcreator-book-header__title' ),
            'stylable'  => true,
            'description' => __( 'Definisci lo stile del titolo iniziale nel file ePub generato.', 'bookcreator' ),
        ),
        'book_subtitle' => array(
            'label'     => __( 'Sottotitolo del libro', 'bookcreator' ),
            'selectors' => array( '.bookcreator-book-header__subtitle' ),
            'stylable'  => true,
        ),
        'book_author' => array(
            'label'     => __( 'Autore principale', 'bookcreator' ),
            'selectors' => array( '.bookcreator-book-header__author' ),
            'stylable'  => true,
        ),
        'book_coauthors' => array(
            'label'     => __( 'Coautori', 'bookcreator' ),
            'selectors' => array( '.bookcreator-book-header__coauthors' ),
            'stylable'  => true,
        ),
        'book_publisher' => array(
            'label'     => __( 'Editore', 'bookcreator' ),
            'selectors' => array( '.bookcreator-book-header__publisher' ),
            'stylable'  => true,
        ),
        'book_publisher_logo' => array(
            'label'                 => __( 'Logo editore', 'bookcreator' ),
            'selectors'             => array( '.bookcreator-book-header__publisher-logo-image', '.bookcreator-book-header__publisher-logo img' ),
            'stylable'              => true,
        ...
        'book_language' => array(
            'label'     => __( 'Lingua', 'bookcreator' ),
            'selectors' => array( '.bookcreator-book-header__language' ),
        ),
        'book_description' => array(
            'label'     => __( 'Descrizione', 'bookcreator' ),
            'selectors' => array( '.bookcreator-book-header__description' ),
            'stylable'  => true,
        ),
        'book_frontispiece_extra' => array(
            'label'     => __( 'Contenuti extra introduttivi', 'bookcreator' ),
            'selectors' => array( '.bookcreator-book-header__extra' ),
            'stylable'  => true,
        ),
        'book_cover' => array(
...
function bookcreator_handle_epub_designer_save() {
    if ( ! current_user_can( 'bookcreator_manage_templates' ) ) {
        wp_die( esc_html__( 'Non hai i permessi per salvare i template.', 'bookcreator' ) );
    }

    check_admin_referer( 'bookcreator_save_epub_template', 'bookcreator_save_epub_template_nonce' );
    ...
    $designer_settings = bookcreator_normalize_epub_designer_settings( $designer_settings_raw );
    ...
    $templates[ $template_id ] = array(
        ...
        'type'     => 'epub',
        'owner'    => $owner_id,
        'settings' => bookcreator_normalize_template_settings( $settings, 'epub' ),
    );
...
function bookcreator_render_epub_templates_overview( $templates, $default_url ) {
    ...
    echo '<a class="button button-primary" href="' . esc_url( $designer_url ) . '">' . esc_html__( 'Crea nuovo template', 'bookcreator' ) . '</a>';
    echo '</div>';

    if ( empty( $templates ) ) {
        echo '<p>' . esc_html__( 'Non sono presenti template ePub. Crea un nuovo template utilizzando l’ePub Template Designer.', 'bookcreator' ) . '</p>';
    ...
    echo '<th scope="col">' . esc_html__( 'Ultimo aggiornamento', 'bookcreator' ) . '</th>';
    ...
    foreach ( $templates as $template ) {
        ...
        $designer    = bookcreator_get_template_designer_settings( $template );
        $updated     = '';

        if ( $designer && ! empty( $designer['updated'] ) ) {
            $timestamp = strtotime( $designer['updated'] );
            if ( $timestamp ) {
                $updated = date_i18n( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), $timestamp );
            }
        }

        if ( ! $updated && ! empty( $template['updated'] ) ) {
            $timestamp = strtotime( $template['updated'] );
            if ( $timestamp ) {
                $updated = date_i18n( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), $timestamp );
            }
        }
        ...
        echo '<td>' . ( $updated ? esc_html( $updated ) : '—' ) . '</td>';
        echo '<td class="column-actions">';

        if ( $template_id && bookcreator_current_user_can_manage_template( $template ) ) {
            echo '<a class="button button-secondary" href="' . esc_url( $edit_url ) . '">' . esc_html__( 'Modifica', 'bookcreator' ) . '</a> ';

            echo '<form method="post" class="inline" style="display:inline">';
            ...
            echo '<input type="hidden" name="bookcreator_template_type" value="epub" />';
            echo '<input type="hidden" name="bookcreator_template_id" value="' . esc_attr( $template_id ) . '" />';
            $confirm = esc_js( __( 'Eliminare definitivamente questo template?', 'bookcreator' ) );
            submit_button( __( 'Elimina', 'bookcreator' ), 'link-delete', '', false, array( 'onclick' => "return confirm('" . $confirm . "');" ) );
            echo '</form>';
        } else {
            echo esc_html__( 'Non hai i permessi per modificare questo template.', 'bookcreator' );
        }
...
function bookcreator_render_templates_page( $current_type ) {
    ...
    if ( 'epub' === $current_type && 'list' === $action ) {
        bookcreator_render_epub_templates_overview( $templates, $default_url );
        echo '</div>';
...
function bookcreator_render_epub_designer_page() {
    ...
    if ( $template && ( isset( $template['type'] ) ? $template['type'] : 'epub' ) !== 'epub' ) {
        $template_id = '';
        $template    = null;
    }

    $designer_settings = $template ? bookcreator_get_template_designer_settings( $template ) : bookcreator_normalize_epub_designer_settings( array() );
...
function bookcreator_build_template_styles( $template = null, $type = 'epub' ) {
    ...
    if ( 'pdf' === $type ) {
        ...
        $styles = array(
            ...
            '  height: auto;',
            '}',
            '.bookcreator-book-header__publisher-logo {',
            '  text-align: center;',
            '  margin: 10mm 0;',
            '}',
    ...
    $fields_requiring_font_cascade = array(
        'book_description',
        'book_copyright',
        'book_index',
        'chapter_content',
    ...
    foreach ( $stylable_fields as $field_key => $field ) {
        ...
        $cascade_font = in_array( $field_key, $fields_requiring_font_cascade, true );
        $descendant_selectors = array();
        if ( $cascade_font ) {
            foreach ( $selectors as $selector ) {
                $selector = trim( $selector );
...
function bookcreator_prepare_epub_inline_content( $content ) {
    if ( '' === $content || null === $content ) {
        return '';
    }

    $filtered = force_balance_tags( $content );
...
function bookcreator_build_nav_document( $book_title, $chapters, $language = 'en', $template_texts = null, $heading_visible = true ) {
    $language      = $language ? strtolower( str_replace( '_', '-', $language ) ) : 'en';
    $language_attr = bookcreator_escape_xml( $language );
    ...
    $heading_markup = $heading_visible ? '<h1 class="bookcreator-book__index-title">' . $heading . '</h1>' : '';
 ...
 * @return string
 */
function bookcreator_build_epub_preface_index( $chapters_data, $language = '', $template_texts = null, $heading_visible = true ) {
...
function bookcreator_create_epub_from_book( $book_id, $template_id = '', $target_language = '' ) {
    ...
    $toc_heading_visible = bookcreator_epub_designer_field_visible( $designer_settings, 'toc_heading', true );

    $template_texts = bookcreator_get_all_template_texts( $language );

    $cover_section_title         = isset( $template_texts['cover_title'] ) ? $template_texts['cover_title'] : __( 'Copertina', 'bookcreator' );
    $book_header_section_title   = isset( $template_texts['frontispiece_title'] ) ? $template_texts['frontispiece_title'] : __( 'Frontespizio', 'bookcreator' );
...
XML;
    ...
    $book_header_body  = '<header class="bookcreator-book-header">';

    $author_visible    = bookcreator_epub_designer_field_visible( $designer_settings, 'bc_author', (bool) $author );
    $coauthors_visible = bookcreator_epub_designer_field_visible( $designer_settings, 'bc_coauthors', (bool) $coauthors );

    $header_elements = array();

    if ( $author && $author_visible ) {
        $header_elements['bc_author'] = '<p class="bookcreator-book-header__author">' . esc_html( $author ) . '</p>';
    }

    if ( $coauthors && $coauthors_visible ) {
        $header_elements['bc_coauthors'] = '<p class="bookcreator-book-header__coauthors">' . esc_html( $coauthors ) . '</p>';
    }

    if ( $title && bookcreator_epub_designer_field_visible( $designer_settings, 'post_title', true ) ) {
        $header_elements['post_title'] = '<h1 class="bookcreator-book-header__title bookcreator-book-title">' . esc_html( $title ) . '</h1>';
    }

    if ( $subtitle && bookcreator_epub_designer_field_visible( $designer_settings, 'bc_subtitle', true ) ) {
        $subtitle_markup = bookcreator_prepare_epub_inline_content( $subtitle );
        if ( $subtitle_markup ) {
            $header_elements['bc_subtitle'] = '<p class="bookcreator-book-header__subtitle">' . $subtitle_markup . '</p>';
        }
    }

    if ( $publisher_logo_id && bookcreator_epub_designer_field_visible( $designer_settings, 'publisher_image', true ) ) {
        $logo_url = wp_get_attachment_url( $publisher_logo_id );
        if ( $logo_url ) {
            $alt_text                         = $publisher ? $publisher : __( 'Logo editore', 'bookcreator' );
            $header_elements['publisher_image'] = '<div class="bookcreator-book-header__publisher-logo"><img class="bookcreator-book-header__publisher-logo-image" src="' . esc_url( $logo_url ) . '" alt="' . esc_attr( $alt_text ) . '" /></div>';
        }
    }

    if ( $publisher && bookcreator_epub_designer_field_visible( $designer_settings, 'bc_publisher', true ) ) {
        $header_elements['bc_publisher'] = '<p class="bookcreator-book-header__publisher">' . esc_html( $publisher ) . '</p>';
    }

    if ( $description_meta && bookcreator_epub_designer_field_visible( $designer_settings, 'bc_description', true ) ) {
        $header_elements['bc_description']  = '<section class="bookcreator-book-header__description">';
        $header_elements['bc_description'] .= bookcreator_prepare_epub_content( $description_meta );
        $header_elements['bc_description'] .= '</section>';
    }

    foreach ( $designer_order as $field_id ) {
        if ( isset( $header_elements[ $field_id ] ) ) {
            $book_header_body .= $header_elements[ $field_id ];
            unset( $header_elements[ $field_id ] );
        }
    }

    if ( $header_elements ) {
        foreach ( $header_elements as $markup ) {
            $book_header_body .= $markup;
        }
    }

    if ( $custom_frontispiece ) {
        $book_header_body .= '<section class="bookcreator-book-header__extra">';
        $book_header_body .= bookcreator_prepare_epub_content( $custom_frontispiece );
        $book_header_body .= '</section>';
    }

    $book_header_body .= '</header>';
    $book_header_body  = bookcreator_process_epub_images( $book_header_body, $assets, $asset_map );

    $chapters[] = array(
        'id'       => 'book-header',
        'title'    => $book_header_section_title,
        'filename' => 'book-header.xhtml',
        'href'     => 'book-header.xhtml',
        'content'  => bookcreator_build_epub_document( $book_header_section_title, $book_header_body, $language ),
        'children' => array(),
    );
    ...
    if ( $isbn && bookcreator_epub_designer_field_visible( $designer_settings, 'bc_isbn', true ) ) {
        $copyright_items[] = array(
            'label' => __( 'ISBN', 'bookcreator' ),
            'value' => $isbn,
        );
    ...
    if ( bookcreator_epub_designer_field_visible( $designer_settings, 'bc_copyright', (bool) $copyright_items || $legal_notice ) && ( $copyright_items || $legal_notice ) ) {
        ...
        if ( bookcreator_epub_designer_field_visible( $designer_settings, 'copyright_title', true ) ) {
    ...
    if ( $dedication && bookcreator_epub_designer_field_visible( $designer_settings, 'bc_dedication', true ) ) {
        ...
        if ( bookcreator_epub_designer_field_visible( $designer_settings, 'dedication_title', true ) ) {
    ...
    $preface_content_visible = $preface && bookcreator_epub_designer_field_visible( $designer_settings, 'bc_preface', true );
    ...
    if ( $preface_content_visible || $toc_visible ) {
        ...
        if ( bookcreator_epub_designer_field_visible( $designer_settings, 'preface_title', true ) ) {
        ...
        if ( $preface_content_visible ) {
            $preface_body .= bookcreator_prepare_epub_content( $preface );
        }

        if ( $toc_visible ) {
            $preface_body .= bookcreator_build_epub_preface_index( $ordered_chapters, $language, $template_texts, $toc_heading_visible );
        }
    ...
    if ( $ordered_chapters ) {
        foreach ( $ordered_chapters as $index => $chapter_data ) {
            ...
            $chapter_title_visible   = bookcreator_epub_designer_field_visible( $designer_settings, 'chapter_title', true );
            $chapter_content_visible = bookcreator_epub_designer_field_visible( $designer_settings, 'chapter_content', true );
            $paragraph_title_visible = bookcreator_epub_designer_field_visible( $designer_settings, 'paragraph_title', true );
            $paragraph_content_visible = bookcreator_epub_designer_field_visible( $designer_settings, 'paragraph_content', true );
            $footnotes_visible       = bookcreator_epub_designer_field_visible( $designer_settings, 'bc_footnotes', true );
            $citations_visible       = bookcreator_epub_designer_field_visible( $designer_settings, 'bc_citations', true );

            if ( $chapter_title_visible ) {
            ...
            if ( $chapter_content && $chapter_content_visible ) {
            ...
            if ( ! empty( $chapter_data['paragraphs'] ) ) {
                foreach ( $chapter_data['paragraphs'] as $paragraph_data ) {
                    ...
                    if ( $paragraph_content && $paragraph_content_visible ) {
                    ...
                    if ( $footnotes && $footnotes_visible ) {
                    ...
                    if ( $citations && $citations_visible ) {
    ...
    $final_section_title_fields = array(
        'bc_appendix'     => 'appendix_title',
        'bc_bibliography' => 'bibliography_title',
        'bc_author_note'  => 'author_note_title',
    );

    $final_sections_entries = array();
    foreach ( $final_sections as $meta_key => $section ) {
        ...
        if ( ! bookcreator_epub_designer_field_visible( $designer_settings, $meta_key, true ) ) {
            continue;
        }

        $section_body  = '<div class="bookcreator-section bookcreator-section-' . esc_attr( $meta_key ) . '">';
        $title_field   = isset( $final_section_title_fields[ $meta_key ] ) ? $final_section_title_fields[ $meta_key ] : '';
        if ( ! $title_field || bookcreator_epub_designer_field_visible( $designer_settings, $title_field, true ) ) {
        ...
        $final_sections_entries[ $meta_key ] = array(
            'id'       => $section['id'],
            'title'    => $section['title'],
    ...
    $chapters_output = $chapters;
    $chapters        = $chapters_output;
    $chapters_added  = false;
    $preface_added   = false;
    $dedication_added = false;
    $copyright_added = false;

    foreach ( $designer_order as $field_id ) {
        switch ( $field_id ) {
            case 'bc_copyright':
                if ( ! $copyright_added && $copyright_entry ) {
                    $chapters[]    = $copyright_entry;
                    $copyright_added = true;
                }
                break;
            case 'bc_dedication':
                if ( ! $dedication_added && $dedication_entry ) {
                    $chapters[]     = $dedication_entry;
                    $dedication_added = true;
                }
                break;
            case 'bc_preface':
            case 'table_of_contents':
                if ( ! $preface_added && $preface_entry ) {
                    $chapters[]  = $preface_entry;
                    $preface_added = true;
                }
                break;
            case 'chapter_title':
            ...
            case 'bc_footnotes':
            case 'bc_citations':
                if ( ! $chapters_added && $content_chapters ) {
                    $chapters       = array_merge( $chapters, $content_chapters );
                    $chapters_added = true;
                }
                break;
            case 'bc_acknowledgments':
            case 'bc_appendix':
            case 'bc_bibliography':
            case 'bc_author_note':
                if ( isset( $final_sections_entries[ $field_id ] ) ) {
                    $chapters[] = $final_sections_entries[ $field_id ];
                    unset( $final_sections_entries[ $field_id ] );
                }
                break;
        }
    }
    ...
    if ( $final_sections_entries ) {
        foreach ( $final_sections_entries as $entry ) {
            $chapters[] = $entry;
        }
    }

    $nav_document = bookcreator_build_nav_document( $title, $chapters, $language, $template_texts, $toc_heading_visible );
...
function bookcreator_normalize_percent_value( $value ) {
    if ( null === $value ) {
        return '';
    }
    ...
    $numeric = bookcreator_sanitize_numeric_value( $raw );
    if ( '' === $numeric ) {
        return '';
    }
...
function bookcreator_generate_pdf_from_book( $book_id, $template_id = '', $target_language = '' ) {
    ...
    $should_insert_publisher_break = $publisher_is_visible;
    ...
    if ( '' !== $trimmed_title ) {
        ...
        $header_font_size = max( 8, min( 18, $header_font_size * 0.8 ) );
        $css              .= "\n.bookcreator-pdf-page-header {\n";
        $css              .= '  text-align: center;' . "\n";
        $css              .= '  font-size: ' . number_format( $header_font_size, 2, '.', '' ) . "pt;\n";
        $css              .= "  margin: 0;\n";
        $css              .= "  padding: 6pt 0;\n";
        $css              .= "  border-bottom: 0.2pt solid #cccccc;\n";
        $css              .= "  color: #555555;\n";
    ...
    $book_header_html  = '<header class="bookcreator-book-header">';

    if ( $author || $coauthors ) {
        $book_header_html .= '<div class="bookcreator-book-header__authors">';

        if ( $author ) {
            $book_header_html .= '<p class="bookcreator-book-header__author">' . esc_html( $author ) . '</p>';
        }

        if ( $coauthors ) {
            $book_header_html .= '<p class="bookcreator-book-header__coauthors">' . esc_html( $coauthors ) . '</p>';
        }

        $book_header_html .= '</div>';
    }

    $book_header_html .= '<h1 class="bookcreator-book-header__title bookcreator-book-title">' . esc_html( $title ) . '</h1>';

    if ( $subtitle ) {
        $subtitle_markup = bookcreator_prepare_epub_inline_content( $subtitle );
        if ( $subtitle_markup ) {
            $book_header_html .= '<p class="bookcreator-book-header__subtitle">' . $subtitle_markup . '</p>';
        }
    }
    ...
    $description_html = '';
    if ( $description_meta ) {
        $prepared_description = bookcreator_prepare_epub_content( $description_meta );
        if ( $prepared_description ) {
            $description_html  = '<section class="bookcreator-book-header__description">';
            $description_html .= $prepared_description;
    ...
    $custom_front_html = '';
    if ( $custom_front ) {
        $prepared_front = bookcreator_prepare_epub_content( $custom_front );
        if ( $prepared_front ) {
            $custom_front_html  = '<section class="bookcreator-book-header__extra">';
            $custom_front_html .= $prepared_front;
            $custom_front_html .= '</section>';
        }
    }
